...
export const Register = ({ setActiveTab }) => {
  ...
  // Authentification / Inscription via Google OAuth
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.response?.data?.message || "Erreur d'inscription Google.");
    } finally {
      setLoading(false);
    }
  };

  // Authentification / Inscription via GitHub OAuth
  const handleGitHubRegister = async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await axios.get(`/v1/auth/github/redirect?json=true&_t=${new Date().getTime()}`);
      if (response.data?.url) {
        window.location.href = response.data.url;
      } else {
        setError("Impossible d'initialiser l'inscription GitHub.");
      }
    } catch (err) {
      setError(err.response?.data?.error || err.response?.data?.message || "Erreur d'inscription GitHub.");
    } finally {
      setLoading(false);
    }
  };
        ...
        {registeredData ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', margin: '20px 0' }}>
              ...
              <span style={{ padding: '0 12px', fontSize: '12px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 600 }}>
                ou s'inscrire avec
              </span>
              <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--border-color)' }}></div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              <button 
                ...
                disabled={loading}
                style={{ flex: 1, minWidth: '140px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontWeight: 600, padding: '10px 14px' }}
              >
